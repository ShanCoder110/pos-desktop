...
export { fifoLot, fifoLots } from "@/pages/products/productFifo";
...
export function nextLotNumber(lots: ProductLotRow[]) {
  const nums = lots
    .map((l) => Number(String(l.lotNumber).replace(/\D/g, "")))
...
export function syncProductStock(
  products: Product[],
  lots: ProductLotRow[],
  productId: string,
): Product[] {
  ...
  return products.map((p) =>
    p.id === productId ? { ...p, stock: totals.stock, damaged: totals.damaged } : p,
  );
}
