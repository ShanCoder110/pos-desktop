  ...
  return (
    <div className="ui-kpi-row [display:grid] [grid-template-columns:repeat(5,_minmax(0,_1fr))] [gap:10px] [width:100%] [flex-shrink:0]">
      ...
      <KpiCard
        label="Paid"
        value={paid.length}
        hint={money(paid.reduce((s, r) => s + r.paidAmount, 0))}
        tone="ok"
        icon={<CheckCircle2 size={16} />}
        active={sectionKpi === "paid"}
        onClick={() => toggle("paid")}
      />
      <KpiCard
        label="Partial"
        value={partial.length}
        hint="Still collecting"
        tone="stale"
        icon={<Clock size={16} />}
        active={sectionKpi === "partial"}
        onClick={() => toggle("partial")}
      />
      <KpiCard
        label="On credit"
        value={credit.length}
        hint={money(completed.reduce((s, r) => s + r.creditAmount, 0))}
        tone="warn"
        icon={<Wallet size={16} />}
        active={sectionKpi === "credit"}
        onClick={() => toggle("credit")}
      />
      <KpiCard
        label="Cancelled"
        value={cancelled.length}
        hint="No stock change"
        tone="danger"
        icon={<Ban size={16} />}
  ...
  return (
    <div className="ui-kpi-row [display:grid] [grid-template-columns:repeat(5,_minmax(0,_1fr))] [gap:10px] [width:100%] [flex-shrink:0]">
      ...
      <KpiCard
        ...
        hint="SKUs with damage"
        tone="danger"
        icon={<Ban size={16} />}
        active={sectionKpi === "damaged"}
        onClick={() => toggle("damaged")}
      />
      <KpiCard
        label="Damage qty"
        value={damageCount}
        hint="Units written off"
        tone="danger"
        icon={<Ban size={16} />}
      />
      <KpiCard
