...
function chartHasData(data: HubChartPoint[]) {
  return data.some((point) => Number.isFinite(point.value));
}

export function HubChartGrid({ children }: { children: ReactNode }) {
  const charts = Children.toArray(children).filter(
    (child): child is ReactElement<{ data: HubChartPoint[] }> =>
      isValidElement(child) && Array.isArray((child.props as { data?: HubChartPoint[] }).data),
  );
  const visible = charts.filter((child) => chartHasData(child.props.data));
  if (!visible.length) {
    return (
      <EmptyState
        className="ui-empty-state-fill"
        icon={<BarChart3 size={18} />}
        title={EMPTY_COPY.filter}
      />
    );
  }
  return <div className="ui-hub-chart-grid">{visible}</div>;
}
...
export function HubChart({
  ...
  const shown =
    type === "bar" ? (maxItems === null ? valid : valid.slice(0, maxItems)) : valid.slice(0, 120);
          ...
          <header className="mb-5 flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-[10px] text-muted">
                {subtitle ?? `${shown.length} filtered items`}
              </p>
            </div>
            {controls}
          </header>
          ...
          {type === "bar" ? (
            <div className="grid max-w-5xl gap-3">
              {shown.map((point, index) => (
                ...
                >
                  <span className="truncate text-[11px] font-semibold text-sub" title={point.label}>
                    {point.label}
                  </span>
                  <div className="h-7 overflow-hidden rounded-md bg-bg">
                    <div
                      ...
                      style={{
                        width: `${Math.max(1, (point.value / max) * 100)}%`,
                        background: CHART_COLORS[index % CHART_COLORS.length],
                      }}
                    />
                  </div>
                  <strong className="text-right text-[11px] tabular-nums text-ink">
                    {formatValue(point.value)}
                  </strong>
                  ...
                  return (
                    ...
                    >
                      <circle
                        cx={x}
                        cy={y}
                        r={selectedId === point.id || showLabel ? "5" : "3"}
                        fill="var(--paper)"
                        stroke={color}
                        strokeWidth={showLabel ? "3" : "2"}
                      >
                        <title>{pointTitle(point, formatValue)}</title>
                      </circle>
                      {showLabel ? (
                        <text x={x} y="302" textAnchor="middle" fontSize="10" fill="var(--muted)">
          ...
          {type === "donut" ? (
            <div className="grid max-w-4xl grid-cols-[260px_1fr] items-center gap-8">
              <div
                ...
                style={{
                  background: `conic-gradient(${shown
                    .map((point, index) => {
                      const before = shown
                        .slice(0, index)
                      ...
                      const end = total
                        ? ((before + Math.max(0, point.value)) / total) * 100
                        : start;
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                {shown.map((point, index) => (
                  ...
                  >
                    ...
                    />
                    <span className="truncate text-sub" title={point.label}>
                      {point.label}
                    </span>
