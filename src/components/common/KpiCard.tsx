...
export function KpiCard({
  ...
  const className = cn(
    "ui-kpi [position:relative] [display:block] [width:100%] [padding:12px_14px_14px] [background:var(--paper)] [border:1px_solid_var(--line)] [border-radius:10px] [box-shadow:0_1px_2px_color-mix(in_srgb,var(--ink)_4%,transparent)] [overflow:hidden] [text-align:left] [cursor:default] [font-family:inherit] [color:inherit] [transition:transform_0.15s_ease,_box-shadow_0.15s_ease]",
    `is-${tone}`,
    onClick && "is-click",
    active && "is-on",
  );
